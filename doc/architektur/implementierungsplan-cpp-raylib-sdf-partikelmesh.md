...
## 15. Statusübersicht (Stand 2026-09-15)
...
|---|---|---|
| 1 Projektgerüst und Kamera | ✅ umgesetzt | CMake + FetchContent (raylib 5.5, GLM 1.0.1, ImGui 1.92.7, rlImGui), CMake-Presets (debug/asan/**release**, Ninja) und VSCode-Launch mit `preLaunchTask` je Build-Variante; Fenster resizable + F11; Kugel-SDF, Bounding Box, Achsen; Kamera via Maus (Y-invertiert), WASD/Pfeiltasten, Scroll/Zoom |
...
| 3 Spatial Hash und Nachbarn | ✅ umgesetzt | `cellSize = repulsionRadius`, 27 Nachbarzellen, deduplizierte Paare `j>i`; Paaranzahl im UI, min/avg/max Abstand + StdAbw. als Metrik; Referenz-Verifikation gegen O(N²)-Suche (`tests/test_spatialhash_reference.cpp`), Nachbarlinien-Overlay für selektierten Partikel; **vollständiges Grid-Overlay aller belegten Zellen** (`SceneRenderer::drawSpatialGrid`, Toggle „Grid (alle Zellen)“); **`build()` parallel** (`49dfd7f`): 2-Phasen-Aufbau (pro-Worker-Teil-Maps + Merge, dann Paargenerierung read-only über Zellen) ab N≥1024 mit Pool, sonst serielle Referenz |
| 4 Tangentiale Relaxation | ◐ weitgehend | Tangentiale Repulsion `f(d)=k(1-d/R)²/d`, gedämpfte Integration, Verschiebungs-Clamp (`maxStepLength·h`), SDF-Projektion pro Substep; **partikelparallel über `ThreadPool::parallelFor`** (`1170354`, jeder Partikel schreibt nur `velocity[i]`, Nachbarsuche read-only — keine Races); **Dämpfung als Impuls-Erhaltung** (`0a27434`, `v *= damping` statt Kraft-Skalierung, tangentialer Anteil `v_t = v − n(n·v)`); Einzelschritt, Partikel-Abstands-Heatmap, Live-Abstands-Histogramm, Kraftpfeil (gesamt=gelb, tangential=magenta) für selektierten Partikel, Bewegungs-Trail, `avg/max v` als Konvergenz-Indikator; **Vorher-/Nachher-Projektionslinien umgesetzt** (`SceneRenderer::drawSDFProjections`, Cyan, Toggle „SDF-Projektion“); Konvergenzkriterium aus Abstands-Streuung und Trails für Stichprobe fehlen |
| 5 Initialtriangulation und Mesh-Rendering | ✅ Abnahme erfüllt | Fan-Triangulation (Tangentialprojektion + `atan2`-Sortierung), Kantenfilter (Länge, Normalenwinkel, SDF-Midpoint), Orientierungsvereinheitlichung, Dedup + Manifold-Prüfung (Kante max. 2×), finale Validierung; **Ueberarbeitung** (`6f6482c`): Normalen-/Midpoint-Vorfilter entfernt; **Boundary-Loop-Fill entfernt** (`34632b2`): A/B-Vergleich (7 Formen × 8 Seeds, identische Partikelpositionen) zeigte, dass `build()` selbst geschlossene Meshes erzeugt — der Loch-Fill war seit der Delaunay-artigen Priorisierung inert und wurde entfernt (Details s. „Bekannte Einschränkung: Mesh-Lücken nach Relaxation“); **Kandidatensuche parallelisiert** (`e20aef9`): der O(N²)-Nachbarsuch-Anteil der Fan-Erzeugung läuft ab N≥1024 partikelparallel (pro-Thread Kandidaten-/Stats-Puffer, serielles Merge am Ende), `tri.build(..., &system.pool())`; Mesh flächengefüllt + Wireframe-overlay + Partikel-Toggle, **Mesh-Qualitäts-Overlay** (Dreiecksfarbe nach Minimalwinkel, Poor rot, Schwellwert einstellbar), Vertices folgen pro Frame `particles[i].position`. Spacing-Formel `sqrt(area/N)`. Nach 60 s Relaxation: `F=2V−4`, `χ=2`, **0 Randkanten** über 10 Seeds (App-Defaults) |

### Plan-Test 4: Konkave CSG-Ausnehmung (weiche U-Form) — umgesetzt

Umsetzungsplan-Test 4 „Konkave CSG-Form“ ist als **Kugel minus versetzte Kugel** mit weicher Überblendung implementiert (`7770130`):

- `SphereMinusSphereSDF` (`PrimitiveSDF.hpp/.cpp`) mit Smooth-Max-Parameter `smoothK` (k→0 ≈ harte Gratkante). Quílez-Blend: `h = clamp(0.5 + 0.5·(a−b)/k, 0, 1)`, `φ = b + h·(a−b) + k·h(1−h)` (Pluszeichen = Smooth-Max), Gradient `∇φ = h·∇a + (1−h)·∇b` (a=φA, b=−φB); **am Schnittkreis gilt exakt `φ = +k/4`**.
- `computeProfile()` als **Marching-Squares-Kontur** von φ(x,r,0)=0 im (x,r)-Querschnitt; `surfaceArea() = 2π·∫r·ds` über beide Wand-Äste (der Querschnitt ist im Überlapp hohl) → generisches Spacing `sqrt(A/N)` und `boundsMin/Max`.
- UI: Primitiv **„Kugel-minus-Kugel“** mit Slidern Basis-Radius, Schnitt-Radius, Versatz, **Smooth k**; echte Ausnehmung nur für `|R−r| < offset < R+r`.
- Tests: `tests/test_closed_shapes.cpp` „CSG-U-Form“ (k=0.2, N=1500): **Rand=0, orient=0, minWinkel ≈41°, maxAspect ≈1.5** — die weiche Form schließt die Fan-Triangulation (die harte Gratkante ~116° ließ einen Loop offen); `tests/test_sdf_primitives.cpp`: k=0.01 (quasi-hart) flächentreu, Blend-Sanity (k=0.5: φ=+k/4 am Schnittkreis).

### Parallelisierung (CPU-Mehrkern)

Dependency-freier eigener **`ThreadPool`** (`src/core/ThreadPool.hpp/.cpp`), gemeinsam von Simulation und Mesh-Erzeugung genutzt (macOS-libc++ liefert `std::execution::par` nur seriell). Haupt-Thread arbeitet mit; `parallelFor` vergibt Indizes per `atomic_fetch_add`; `currentWorkerId()` (`thread_local`) adressiert pro-Worker-Puffer (Teil-Maps, Teil-Paarlisten, Kandidaten).

| Baustein | Ansatz | Schwellwert |
|---|---|---|
| `ParticleSystem::relax()` / `projectToSDF()` (`1170354`) | Kraftschleife partikelparallel; jeder Partikel schreibt nur `velocity[i]`, Nachbarsuche read-only über `SpatialHash::idsInCell()` (27 Nachbarzellen) — kein Race, keine Atomics/Reduce, Paar wird dafür 2× statt 1× evaluiert | immer parallel (Pool) |
| `SpatialHash::build()` (`49dfd7f`) | 2 Phasen: pro-Worker-Teil-Maps + serielles Merge, danach Paargenerierung read-only über Zellen | ab N ≥ 1024 + Pool; sonst serielle Referenz |
| `Triangulation::build()` Fan-Kandidaten (`e20aef9`) | O(N²)-Nachbarsuche partikelparallel, pro-Thread Kandidaten-/Stats-Puffer (kein Vector-Race), serielles Merge am Ende; `rebuildTopology()` reicht `&system.pool()` durch | ab N ≥ 1024 + Pool (`pool == nullptr` → serial) |

- `tests/test_spatialhash_reference.cpp`: parallel N=2000 == serielle O(N²)-Referenz (43.103 Paare, identisches Set; ferner serial N=200/2000). `pairs()`-**Reihenfolge ist bei Parallelität nicht-deterministisch** (Ergebnissett identisch, unschädlich für Mesh/Renderer/Metriken).
- Ergebnisgleichheit: Meshtests und Spacing-/Topologie-Tests laufen unverändert PASS; Parallelität ändert nur die Paar-/Kandidaten-Reihenfolge.

### Debug-UI

Umgesetzt: Pause/Weiter (**Button oder Leertaste**, `a66d15b`), **Einzelschritt**, Reset, „Triangulation erzeugen", **Auto-Rebuild (Debug)** mit Frames-Intervall (trianguliert während der laufenden Simulation periodisch neu; unterbricht bewusst die persistente Topologie), FPS, Partikelzahl, Paaranzahl, Dreiecks-Stats; Partikel-Slider im „SDF-Form“-Header (1000–10000, Neuinitialisierung), Primitiv-Combo + Formparameter-Slider inkl. **Smooth k** (Metaball/CSG); Toggles Mesh/Wireframe/Partikel/**Partikel-Heatmap**/**Mesh-Qualität**/Achsen/Bounding Box/**Grid (alle Zellen)**/**SDF-Projektion**; Slider für Max Kantenlänge, **Poor-Winkel (Quality-Threshold)**, Repulsion, Stärke, Dämpfung, Substeps, MaxSchritt (Breite begrenzt, damit Labels passen), **Parameter zurücksetzen** bei den Reglern; Clamp-Logik zentral in `applySDFForm` (Doppel-Clamps im UI entfernt); **Off-Center-Ansicht** (`3a61828`): 3D-Hauptansicht wird um einstellbare Pixel nach rechts verschoben (schraeges Frustum ueber `m8` der Projektionsmatrix, tiefenkonstant), Slider „View-Versatz (px)" 0–400; Rechtsklick-Raycast kompensiert den Versatz. Permanente Metriken (`src/debug/Metrics`): Sim/Grid-Zeit, `max/avg |φ(p)|`, Abstand min/avg/max/StdAbw + unter/ok/über `h`, `avg/max v`, Mesh-Qualität (min. Innenwinkel, max. Aspect Ratio, „poor“-Zähler), **Live-Histogramm der Nachbarabstände**, **Topologie-Persistenz** („persistent seit N Frames“). **Partikel-Auswahl per Rechtsklick**: ID, Position, `φ(p)`, Nachbaranzahl; Overlays Grid-Zelle, Nachbarlinien, Kraftpfeil (gesamt/tangential), Normale, Bewegungs-Trail.
...
| `src/core/ThreadPool` | neu hinzugekommen (nicht im Plan): eigener dependency-freier Thread-Pool für die CPU-Mehrkern-Parallelisierung; anfangs in `src/simulation/`, mit `e20aef9` nach `src/core/` ausgelagert, weil Simulation **und** Mesh-Erzeugung ihn nutzen |
| Tests (Plan: Catch2/doctest) | `tests/test_triangulation.cpp` (Euler-Test, Fibonacci-Sphäre), `tests/test_spacing_regression.cpp`, `tests/test_closed_mesh.cpp` (10 Seeds geschlossen, `F=2V−4`), `tests/test_metrics.cpp` (Verteilungs-/SDF-/Mesh-Metriken plausibel), `tests/test_spatialhash_reference.cpp` (Grid-Pfad == O(N²)-Referenz; zusätzlich seriell vs. parallel N=2000, identisches Paarset), `tests/test_persistent_topology.cpp` (M5: persistenter Indexbuffer über +10 s/+60 s/+5 min, Vertex-Drift-Monitor), `tests/test_sdf_primitives.cpp` (M6 Primitiva: Kugel/Ellipsoid/Torus/Hantel, Fächentreue; CSG k=0.01 quasi-hart + Blend-Sanity k=0.5), `tests/test_closed_shapes.cpp` (M6/Metaball + Plan-Test 4 CSG-U-Form: triangulierte Formen geschlossen/bewertet); kein Test-Framework eingebunden |
...
2. **Loch-Schließung:** Die verbleibenden kleinen Lücken (1–2 fehlende Dreiecke, als spitze Löcher sichtbar) entstanden durch die Manifold-Rejection konkurrierender Fan-Kandidaten. Ein damaliger **Boundary-Loop-Fill** in `Triangulation::closeBoundaryLoops()` verdrahtete Randkanten zu Loops und füllte 3- und 4-er Loops gefiltert (F = 2V−4, `χ=2`). **Später entfallen** (`34632b2`): Seit die Delaunay-artige Priorisierung (kompakteste Dreiecke zuerst) die Manifold-Konkurrenz in `build()` selbst entscheidet, sind derartige Lücken nicht mehr aufgetreten — ein A/B-Vergleich (7 Formen × 8 Seeds, identische Partikelpositionen) zeigte bitidentische Ergebnisse mit und ohne Fill, daraufhin wurde die Funktion entfernt.
